...
- 工具调用、脚本执行、阶段性判断和“我现在开始/继续检查/正在生成”等过程日志由客户端活动流展示。
- 最终正文不得重复粘贴上述执行日志，也不得输出逐步思考过程；只保留采集说明、符合规定的 TXT 文本、纯净 JSON 和 HTML 产物链接。
- 不要使用“我先加载”“环境就绪”“继续深挖”“现在生成”“清理临时文件”等过程性句子作为最终回答段落。

### 全部输出的强制来源要求

本技能无论输出 txt、JSON、Markdown、HTML、摘要、图表还是分析结论，都必须为每条文章/数据同时提供发布单位或网站全称、文章来源/页面完整标题和指向具体原文的链接。只写单位、网站名、域名、栏目页或搜索结果页均不合格；无法核验时必须写“来源：未提供/待核验”，严禁编造 URL、标题或单位。
...
最终展示时，`organization`、`title`、`link` 必须组合成完整来源：`来源：单位：《文章标题》`，不能只展示单位名称；不得把多个文章的来源合并成一组单位列表。
