...
4. **机械化无污染写盘**：
   - **文件保存位置与操作**：大模型**必须且只能**调用本地文件写入工具，将每个页面的解析结果 JSON 内容以文件名 `Filename = EXACT_STRING + ".json"` 写入文件。具体保存目录遵循客户端当次会话注入的默认输出规则执行，不要自行发明另一套目录约定。
   - **严禁**自作聪明在文件名添加日期前缀或删减字词，否则触发系统 FATAL ERROR。
   - **对话输出限制（绝对铁律）**：为了保持界面整洁，**严禁在对话回复中直接输出或显示完整的 JSON 文本**。大模型在对话中只需输出“已成功抓取并写入文件：`[相对路径]/[Filename]`”，以及一个极简的 100 字内核心数据结论即可，绝对不要把庞大的 JSON 结构输出在聊天气泡中。
